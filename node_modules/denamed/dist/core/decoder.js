"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.decodeNsName = exports.decodeMessage = exports.decodeResourceRecord = exports.decodeFlags = void 0;
const protocol_1 = require("./protocol");
const ip_1 = require("./ip");
const decodeFlags = (buffer, offset) => {
    const flags = buffer.readUInt16BE(offset);
    return {
        queryResponse: flags & 0x8000 ? 1 : 0,
        operationCode: flags & 0x7800,
        authoritativeAnswer: !!(flags & 0x400),
        truncation: !!(flags & 0x200),
        recursionDesired: !!(flags & 0x100),
        recursionAvailable: !!(flags & 0x80),
        authenticatedData: !!(flags & 0x20),
        checkingDisabled: !!(flags & 0x10),
        responseCode: flags & 0xf,
    };
};
exports.decodeFlags = decodeFlags;
function decodeResourceRecord(buffer, offset) {
    let off = offset;
    const [name, l] = exports.decodeNsName(buffer, off);
    off += l;
    const typeCode = buffer.readUInt16BE(off);
    off += 2;
    const dClass = buffer.readUInt16BE(off);
    off += 2;
    const ttl = buffer.readUInt32BE(off);
    off += 4;
    const length = buffer.readUInt16BE(off);
    off += 2;
    const type = protocol_1.InverseQueryMap[typeCode];
    let record;
    switch (type) {
        case "OPT":
            {
                let optionsLengths = 0;
                const options = {};
                while (optionsLengths < length) {
                    const optCode = buffer.readUInt16BE(off);
                    off += 2;
                    const optLength = buffer.readUInt16BE(off);
                    off += 2;
                    const value = buffer.slice(off, off + optLength);
                    off += optLength;
                    options[optCode] = value;
                    optionsLengths += 4 + optLength;
                }
                record = {
                    type,
                    ttl,
                    name,
                    class: dClass,
                    data: options,
                };
            }
            break;
        case "A":
            {
                const ip = ip_1.decimalToIpV4(buffer.readUInt32BE(off));
                off += 4;
                record = {
                    type,
                    ttl,
                    name,
                    class: dClass,
                    data: {
                        type: "A",
                        target: ip,
                    },
                };
            }
            break;
        case "SOA":
            {
                const [mName, mLength] = exports.decodeNsName(buffer, off);
                off += mLength;
                const [rName, rLength] = exports.decodeNsName(buffer, off);
                off += rLength;
                const serial = buffer.readUInt32BE(off);
                off += 4;
                const refresh = buffer.readUInt32BE(off);
                off += 4;
                const retry = buffer.readUInt32BE(off);
                off += 4;
                const expire = buffer.readUInt32BE(off);
                off += 4;
                const minimum = buffer.readUInt32BE(off);
                off += 4;
                record = {
                    type,
                    ttl,
                    name,
                    class: dClass,
                    data: {
                        type: "SOA",
                        host: mName,
                        admin: rName,
                        serial,
                        refresh,
                        retry,
                        expire,
                        ttl: minimum,
                    },
                };
            }
            break;
        default:
            throw new Error(`Unsupported type ${type} (${typeCode})`);
    }
    return [record, off - offset];
}
exports.decodeResourceRecord = decodeResourceRecord;
function decodeMessage(buffer) {
    const id = buffer.readUInt16BE(0);
    const flags = exports.decodeFlags(buffer, 2);
    const qdCount = buffer.readUInt16BE(4);
    const anCount = buffer.readUInt16BE(6);
    const nsCount = buffer.readUInt16BE(8);
    const srCount = buffer.readUInt16BE(10);
    let offset = 12;
    const questions = new Array(qdCount);
    for (let i = 0; i < qdCount; i++) {
        const [name, length] = exports.decodeNsName(buffer, offset);
        offset += length;
        const type = protocol_1.InverseQueryMap[buffer.readUInt16BE(offset)];
        // skipping class for right now
        offset += 4;
        questions[i] = {
            name,
            type,
        };
    }
    const [answers, authorities, additional] = [
        anCount ? new Array(anCount) : undefined,
        nsCount ? new Array(nsCount) : undefined,
        srCount ? new Array(srCount) : undefined,
    ];
    [answers, authorities, additional].forEach((set) => {
        if (set) {
            for (let i = 0; i < set.length; i++) {
                const [rr, l] = decodeResourceRecord(buffer, offset);
                set[i] = rr;
                offset += l;
            }
        }
    });
    return {
        ...flags,
        id,
        questions,
        answers,
        authorities,
        additional,
    };
}
exports.decodeMessage = decodeMessage;
const decodeNsName = (buffer, offset) => {
    let start = offset;
    const name = [];
    if (buffer.length < offset + 1)
        return ["", 0];
    let readLength = buffer.readUInt8(offset);
    // If this is a pointer (i.e. 0b11xxxxxx), then ref up
    if (readLength & 0b11000000) {
        return [exports.decodeNsName(buffer, buffer.readUInt16BE(offset) & 0x3fff)[0], 2];
    }
    while (readLength != 0x00) {
        // Check if a pointer
        if (readLength & 0b11000000) {
            // End the record with a pointer
            name.push(exports.decodeNsName(buffer, buffer.readUInt16BE(offset) & 0x3fff)[0]);
            offset++;
            break;
        }
        else {
            offset++;
            name.push(buffer.slice(offset, offset + readLength).toString());
            offset = offset + readLength;
        }
        readLength = buffer.readUIntBE(offset, 1);
    }
    return [name.join("."), offset - start + 1];
};
exports.decodeNsName = decodeNsName;
//# sourceMappingURL=decoder.js.map